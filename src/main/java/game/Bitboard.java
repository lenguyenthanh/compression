...
public class Bitboard {
    ...
    public static long slidingAttacks(int square, long occupied, int[] deltas) {

        //System.out.println("slidingAttacks: " + square + "; " + Long.toHexString(occupied));

        long attacks = 0;
        for (int delta : deltas) {
//            System.out.println("deltal: " + delta);

            int sq = square;
            do {
                //System.out.println("while: " + sq + " -> " + Square.distance(sq, sq - delta) + " -> " + Long.toHexString(attacks));
                sq += delta;
                if (sq < 0 || 64 <= sq || Square.distance(sq, sq - delta) > 2)
                    break;
                attacks |= 1L << sq;
                //System.out.println("while: " + sq + " -> " + Long.toHexString(attacks));
            } while (!Bitboard.contains(occupied, sq));
        }

        //System.out.println("final attacks: " + Long.toHexString(attacks) + " - " + Long.toBinaryString(attacks));
        return attacks;
    }

    public static void initMagics(int square, Magic magic, int shift, int[] deltas) {
        long subset = 0;
        do {
    ...
    static {
        // TODO generate ranks & files mask
        for (int i = 0; i < 8; i++) {
            RANKS[i] = 0xffL << (i * 8);
            FILES[i] = 0x0101010101010101L << i;
        }

        // TODO generate all attacks from all squares
